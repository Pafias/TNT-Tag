...
public class RollbackHandler {
    ...
    public static void delete(String world) {
        for (Player player : Bukkit.getWorld(world).getPlayers())
            player.teleport(TNTTag.getInstance().getLobby());
        String rootDirectory = TNTTag.getInstance().getServer().getWorldContainer().getAbsolutePath();
        File worldFolder = new File(rootDirectory + "/" + world);
        Bukkit.unloadWorld(Bukkit.getWorld(world), false);
        deleteFolder(worldFolder);
        TNTTag.getInstance().getServer().getScheduler().runTaskLater(TNTTag.getInstance(), () -> Bukkit.createWorld(new WorldCreator(world.split("_")[0])), 60);
    }
